...
local GiftRecordScene = class("GiftRecordScene", cc.load("mvc").ViewBase)
-- local json = require("framework.json");
function GiftRecordScene:onCreate()
    ...
    -- add title
    local title = cc.Label:createWithSystemFont("礼物记录", "Arial", 40)
    ...
    local fh_wz = ccui.Text:create("< 赠送礼物", "Arial" , 40)
    ...
    fh_wz:addTouchEventListener(function(sender, eventType)
        ...
        elseif eventType == ccui.TouchEventType.ended then
            ...
            if not move then
                require("app.MyApp"):create():enterScene("GiftScene", "moveinl");
            end     
...
function GiftRecordScene:getData( page)
	-- body
...
-- 绘制顶部
function GiftRecordScene:createTop( ... )
	-- body
    ...
    local ms = cc.Label:createWithSystemFont("收礼记录", "Arial", 40);
    ...
    local ms = cc.Label:createWithSystemFont("送礼记录", "Arial", 40);
    chose_img:addChild(ms);
...
function GiftRecordScene:updateChose( ... )
	-- body
	if self.yk == 1 then 
...
-- 绘制底部
function GiftRecordScene:createBottom( ... )
	-- body	
	local layout = ccui.Layout:create();
	self:addChild(layout);
	layout:setAnchorPoint(cc.p(0, 0));
...
function GiftRecordScene:createCenter(info)
	-- body
	-- 更新数据 
...
-- 绘制列表元素
-- @param data = {}
function GiftRecordScene:createCenterItem( data )
	-- body
